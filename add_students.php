...
<?php
    ...
    if(isset($_POST['student_name'],$_POST['roll_no'])) {
        ...
        // validation
        if (empty($name) or empty($usn) or empty($class_name) or preg_match("/[a-z]/i",$usn) or !preg_match("/^[a-zA-Z ]*$/",$name)) {
            if(empty($name))
        ...
        $sql = "INSERT INTO `students` (`name`, `usn`, `class_name`) VALUES ('$name', '$usn', '$class_name')";
