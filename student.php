...
<body>
    <?php
        ...
        // validation
        if (empty($class) or empty($usn) or preg_match("/[a-z]/i",$usn)) {
            if(empty($class))
        ...
        $result_sql=mysqli_query($conn,"SELECT `p1`, `p2`, `p3`, `p4`, `p5`, `marks`, `percentage` FROM `result` WHERE `usn`='$usn' and `class`='$class'");
